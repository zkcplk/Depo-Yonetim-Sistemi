...
public class UrunGelisi extends javax.swing.JFrame {
  ...
  private HashMap<Integer, Uretici> ureticiMap;
  ...
  public UrunGelisi() {
    ...
    // ürün id ve ureticiId'nin bilindiği yerlerde,
    // ürün adı ve üreticiye ait diğer bilgileri elde edebilmek için.
    urunMap = new HashMap<>();
    ureticiMap = new HashMap<>();
    ...
    tabUreticiler.forEach(uretici -> {
      cmbUreticiler.addItem(uretici.getAd());
      ureticiMap.put(uretici.getId(), uretici);
    });
  ...
  private void initComponents() {
    ...
    cmbUreticiler = new javax.swing.JComboBox<>();
    ...
    jLabel1.setText("Depoya Yeni Ürün Girişi");

    jLabel2.setFont(new java.awt.Font("Tahoma", 0, 16)); // NOI18N
    jLabel2.setText("Üretici Adı:");

    cmbUreticiler.setFont(new java.awt.Font("Tahoma", 1, 16)); // NOI18N
    cmbUreticiler.setToolTipText("Ürünlerin geldiği üreticiyi buradan seçebilirsiniz.");
    ...
    tablo.setModel(new javax.swing.table.DefaultTableModel(
      ...
      new String [] {
        "Üretici ID", "ÜRETİCİ", "Ürün ID", "ÜRÜN", "KOLİ ADEDİ", "TOPLAM ADET"
      }
    ) {
    ...
    if (tablo.getColumnModel().getColumnCount() > 0) {
      ...
      tablo.getColumnModel().getColumn(4).setMaxWidth(75);
    }
    ...
    cmbKoli.setToolTipText("İlgili üründen kaç koli geldiğini buradan seçebilirsiniz.");
        ...
        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
          ...
          .addGroup(layout.createSequentialGroup()
            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
              ...
              .addComponent(cmbUreticiler, javax.swing.GroupLayout.PREFERRED_SIZE, 341, javax.swing.GroupLayout.PREFERRED_SIZE))
            .addGap(18, 18, 18)
      ...
      .addGroup(layout.createSequentialGroup()
        ...
        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
          .addGroup(layout.createSequentialGroup()
            ...
            .addComponent(cmbUreticiler, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
            .addGap(18, 18, 18)
  ...
  private void btnEkleGuncelleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEkleGuncelleActionPerformed
    ...
    String ureticiAdi = cmbUreticiler.getSelectedItem().toString().trim();
    int ureticiId = tabUreticiler.get(cmbUreticiler.getSelectedIndex()).getId();
    ...
    cmbUreticiler.setEnabled(false);

    // eklenmek istenen ürün daha önce eklenmişse,
    ...
    for (int i = 0; i < tabModel.getRowCount(); i++) {
      if (urunAdi.equals(tabModel.getValueAt(i, 3).toString())) {
        yeni = false;
        tabModel.setValueAt(ureticiId, i, 0);
        tabModel.setValueAt(ureticiAdi, i, 1);
        tabModel.setValueAt(urunId, i, 2);
        tabModel.setValueAt(urunAdi, i, 3);
        tabModel.setValueAt(koli, i, 4);
        tabModel.setValueAt(toplamAdet, i, 5);
        break;
      }
    }
    ...
    if (yeni) {
      ...
      // bazı değerler tabloda gizli haldedir, gösterilmez. (ureticiId ve urunId gibi)
      tabModel.addRow(new Object[]{ureticiId, ureticiAdi, urunId, urunAdi, koli, toplamAdet});
    }
  ...
  private void btnKaydetActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKaydetActionPerformed
    if (tabModel.getRowCount() < 1) {
      ...
      pdf = PDFCikti.bufferBaslat(unixTime, "DEPOYA ÜRÜN GİRİŞİ");
      pdf += PDFCikti.bufferUreticiEkle(
              ureticiMap.get(Integer.valueOf(tabModel.getValueAt(0, 0).toString())).getAd(),
              ureticiMap.get(Integer.valueOf(tabModel.getValueAt(0, 0).toString())).getAdres(),
              ureticiMap.get(Integer.valueOf(tabModel.getValueAt(0, 0).toString())).getTelefon()
      );
      pdf += PDFCikti.bufferUrunlerILK("Depoya gelen ürünler");

      // Depoya gelen ürünlerin listesi
      ArrayList<GelenUrun> gelenUrunler = new ArrayList<>();
      ...
      for (int i = 0; i < tabModel.getRowCount(); i++) {
        ...
        GelenUrun gelenUrun = new GelenUrun(0, urunId, koli, unixTime, Sabitler.PERSONELID, ureticiId);
        pdf += PDFCikti.bufferUrunler(tabModel.getValueAt(i, 3).toString(), koli);

        gelenUrunler.add(gelenUrun);
        koliToplam += koli;
      }

      pdf += PDFCikti.bufferUrunlerSON(koliToplam);

      UrunGelis geleni = new UrunGelis();

      if (geleni.kabul(gelenUrunler)) {
        ...
        new Thread(new Runnable() {
          @Override
          public void run() {
            ArrayList<DepodakiUrun> gelmisUrunler = geleni.yerlestir(gelenUrunler);
            pdf += PDFCikti.bufferKolilerILK("Kolilere depoda atanan yerler");

            gelmisUrunler.forEach((depodakiUrun) -> {
            ...
            try {
              PDFCikti.pdfYap(pdf, "GelenUrunler", unixTime);
            } catch (FileNotFoundException ex) {
              JOptionPane.showMessageDialog(UrunGelisi.this, "PDF çıktısı oluşturulamadı!", "PDF Hatası!", 0);
              UrunGelisi.this.dispose();
            }

            JOptionPane.showMessageDialog(UrunGelisi.this, "Ürünler depoya başarıyla eklendi!\n"
                    + "PDF çıktısını masaüstünüzde bulabilirsiniz.", "Ürün Ekleme Başarılı ✓", 1);
  ...
  private javax.swing.JComboBox<String> cmbUreticiler;
